...
_n98-magerun2()
{
    ...
    if [[ ${cur} == --* ]] ; then
        ...
        case "$com" in
            ...
            db:dump)
            opts="${opts} --connection --db-name --zstd-level --zstd-extra-args --add-time --compression --only-command --print-only-filename --dry-run --set-gtid-purged-off --no-single-transaction --human-readable --git-friendly --add-routines --no-tablespaces --stdout --strip --exclude --include --force --keep-column-statistics --keep-definer --mydumper --views --no-views"
            ;;

            db:import)
            opts="${opts} --connection --db-name --compression --zstd-level --zstd-extra-args --only-command --only-if-empty --optimize --drop --drop-tables --force --skip-authorization-entry-creation"
            ;;
            ...
            opts="${opts} --original-db --output-file --no-data-diff --compare-extra-arg --connection"
            ;;
