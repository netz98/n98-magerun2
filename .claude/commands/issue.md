...
argument-hint: [bug|feature|enhancement|question|support] <description...>
---
...
- Check if the GitHub MCP server is connected (tools like `mcp__github__issue_write`,
  `mcp__github__search_issues`, `mcp__github__list_issue_types`). Either `gh` or the MCP server is
  enough; if neither is available, stop and tell the user to install/authenticate the `gh` CLI or
...
using `gh issue list --search "..."` / `gh search issues` or the MCP search tool
(`mcp__github__search_issues`). If you find a strong candidate duplicate, show it to the user and
ask whether to proceed, comment on the existing issue instead, or cancel — don't create a duplicate
silently.
...
If `gh` is not available or not authenticated, use the GitHub MCP server's issue tool
(`mcp__github__issue_write` with method `create`) with the equivalent parameters (owner `netz98`,
repo `n98-magerun2`, title, body, labels).
