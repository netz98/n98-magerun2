...
argument-hint: [issue-number] [base-branch] [title override...]
---
...
- Check if a GitHub MCP server is connected (tools like `mcp__github__create_pull_request`,
  `mcp__github__get_me`). Either `gh` or the MCP server is enough; if neither is available, stop and
  tell the user to install/authenticate the `gh` CLI or connect the GitHub MCP server.

## Detect the current GitHub identity

- Via `gh`: `gh api user --jq .login`.
- Via MCP: `mcp__github__get_me`.
...
If no issue number was given in `$ARGUMENTS`, try to detect one automatically:
...
If `gh` is not available or not authenticated, use the connected GitHub MCP server tool
(`mcp__github__create_pull_request`) with the equivalent parameters (owner `netz98`, repo
`n98-magerun2`, base, head, title, body).
